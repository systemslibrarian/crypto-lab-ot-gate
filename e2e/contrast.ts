 ...
 *    files the node under `incomplete`. The DDH candidate points and result
 *    figures sit on translucent surface tints, so their text composites onto
 *    whatever is behind rather than a flat colour.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. Note this lab's fades live
 *    only in `@keyframes` and reduced motion collapses them to 0.01ms, so
 *    nothing strands at `opacity: 0` under the preference.
 *
 ...
 *    the Curve25519 point hex and derived keys are long and monospace, so at
