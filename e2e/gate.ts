 ...
 *     nothing, and this lab gates its own controls: "Make Selection" is
 *     `disabled` until the sender is initialised, so a gate that does not walk
 *     the protocol in order measures a page of disabled buttons and empty
 *     output panels.
 *
...
export async function boot(page: Page, theme: 'dark' | 'light'): Promise<void> {
  ...
  await expect(page.locator('#btn-sender-init')).toBeVisible();
 ...
 * plausible offender: it prints group elements and derived keys as long hex,
 * and lays the privacy audit out on a `minmax(220px, 1fr)` track.
 */
 ...
 *
 * The OT protocol is walked in order because the page enforces that order —
 * "Make Selection" stays disabled until the sender is initialised — and both
 * receiver choices are exercised, since the whole claim of the exhibit is that
 * the sender's view is identical either way.
 */
export async function driveAllStates(
  ...
  await scan(page, `${label} / first paint`);

  await page.locator('#m0-input').fill('message zero');
  await page.locator('#m1-input').fill('message one');
  await page.locator('#btn-sender-init').click();
  await expect(page.locator('#sender-output')).not.toBeEmpty();
  await expect(page.locator('#btn-receiver-choose')).toBeEnabled();
  await scan(page, `${label} / sender initialised`);

  await page.locator('#choice-1').check();
  await page.locator('#btn-receiver-choose').click();
  await expect(page.locator('#receiver-output')).not.toBeEmpty();
  await scan(page, `${label} / receiver chose m1`);

  await page.locator('#btn-correctness').click();
  await expect(page.locator('#correctness-results')).not.toBeEmpty();
  await scan(page, `${label} / correctness verified`);

  // The other branch. A completed transfer disables BOTH the sender and
  // receiver controls — the session is deliberately one-shot, so the only way
  // to see the m0 branch is the way a visitor sees it: reload and run again.
  ...
  await page.locator('#btn-receiver-choose').click();
  await expect(page.locator('#receiver-output')).not.toBeEmpty();
  await scan(page, `${label} / receiver chose m0`);

  // The DDH guessing game. Dealing a round only renders the three candidate
  // points; the exhibit's actual claim — that they are indistinguishable — is
  // only demonstrated once you GUESS, which is what renders the verdict and
  // running tally and reveals the reset control.
  await page.locator('#btn-ddh').click();
  const points = page.locator('#ddh-results .ddh-point');
  await expect(points).toHaveCount(3);
  await scan(page, `${label} / DDH round dealt`);
  ...
  await scan(page, `${label} / DDH guess scored`);

  await page.locator('#btn-ddh-auto').click();
  await expect(page.locator('#ddh-auto')).not.toBeEmpty();
  await scan(page, `${label} / DDH 1000 rounds`);

  await page.locator('#btn-ddh-reset').click();
  await scan(page, `${label} / DDH reset`);
}
